...
| `marathon.html` | the **marathon mode**, a page of its own (it shares the code of the live page, `live.js`; `live.html` has no marathon controls): the list per lap of a race of a group, start time and finish lap, and the replay of a marathon by activity number, with an **Export video** button that records the track while the replay plays (from the chosen start to the end) and downloads it as a `.webm` file, using the browser's own `canvas.captureStream` + `MediaRecorder` (nothing is uploaded anywhere). Linked as "Marathon" in the menu of the main page |
