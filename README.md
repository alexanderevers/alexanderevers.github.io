...
| Page | Features |
|---|---|
| `index.html` (main) | look up a transponder (`XX-12345`; the field adds capitals and the dash by itself and cleans up pasted text); list **all** its activities (up to 500, with a year filter, each with its day of the week: "Tue 01/09/2026 - 20:00 - ..."); session summary tiles and charts; a "lap time threshold" slider that separates speed laps from the rest; speed-lap analysis (average, median, best 5, consistency, fade, speed blocks with distance and totals, distribution); session summary cards (including how much of the session you were really skating); **GPX download for Strava** (for rinks that have a track file); **find overlapping sessions**: every rider who was on the ice during the activity, with *skated together* and *in your group* estimates, sorted by in your group first, then by skated together; a **Marathon analysis** button appears when the fetched sessions look like a marathon (a group of riders starting together after a break), opening `marathon.html` for that activity; a **Records** dashboard remembers every finished session locally in the browser, for one transponder at a time (`history-store.js`; looking up a different transponder offers **Change my transponder** instead of mixing histories, after a warning that the other one is forgotten), and shows the personal best lap ever, per **ice-skating season** (September through April, e.g. "25/26"), over time, and the fastest 20 % of laps per season with their average (`records.js`), with a **Load full history** button, a wide two-column layout with both charts stacked beside the per-season table, click-to-scroll from a chart dot or table row to its session, a **Fetch laps** button per stored session, and the option to exclude or delete a whole session or a single lap (a mistimed crossing, or a session that was not really you) |
| `live.html` | **live**: who is on the ice at a rink right now (default IJsbaan Twente), refreshed every 1-60 s. Riders on the ice (last finish crossing under 2 minutes ago) are moving dots on the track and rows with laps, last and best lap, time since the last crossing, start time and duration of the activity (after the rider's name); after 2 minutes without a crossing a rider moves to "Recently on the ice", after 15 minutes he is gone. Click a rider for the lap-time graph; click a second rider to compare (paler line in the same graph), click that one again to make it the only selection. **Marathon mode** (switch): a list per lap for a race of a group (order of crossing, gap and distance to the first rider, start time and finish lap, everybody who comes in during the finish lap counts); an **activity number** replays a marathon of an earlier day. Needs the Worker with `?live=1` (1 s cache) deployed |
| `marathon.html` | the **marathon mode**, a page of its own (it shares the code of the live page, `live.js`; `live.html` has no marathon controls): the list per lap of a race of a group, start time and finish lap, and the replay of a marathon by activity number, with an **Export video** button that records the track, the riders table and the lap-times/position graph of the selected rider together while the replay plays (from the chosen start to the end) and downloads it as a `.webm` file, using the browser's own `canvas.captureStream` + `MediaRecorder` (nothing is uploaded anywhere). Linked as "Marathon" in the menu of the main page |
